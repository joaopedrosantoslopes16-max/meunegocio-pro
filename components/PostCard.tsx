...
// ── helpers de escala ──────────────────────────────────────
function s(value: number, compact: boolean) {
  return compact ? Math.round(value * 0.52) : value;
}
function px(value: number, compact: boolean) {
  return `${s(value, compact)}px`;
}

// ── Slide container base ────────────────────────────────────
function Slide({ bg, children, compact }: { bg: string; children: React.ReactNode; compact: boolean }) {
  return (
    <div style={{
      aspectRatio: "1/1",
      background: bg,
...
// 1. WHATSAPP CTA
// ══════════════════════════════════════════════════════════
function WhatsAppCTA({ title, subtitle, cta, business_name, color, city, c: compact }: any) {
  const p = (v: number) => px(v, compact);
  const pad = s(32, compact);
  return (
    <Slide bg="#0A0D14" compact={compact}>
      {/* Glow radial */}
      ...
      <svg style={{ position: "absolute", inset: 0, width: "100%", height: "100%", opacity: 0.04 }} viewBox="0 0 400 400">
        {Array.from({ length: 9 }, (_, i) => <line key={`h${i}`} x1="0" y1={i * 50} x2="400" y2={i * 50} stroke="white" strokeWidth="0.5"/>)}
        {Array.from({ length: 9 }, (_, i) => <line key={`v${i}`} x1={i * 50} y1="0" x2={i * 50} y2="400" stroke="white" strokeWidth="0.5"/>)}
      </svg>
      ...
      <div style={{ padding: `${pad}px`, display: "flex", flexDirection: "column", height: "100%", position: "relative", zIndex: 1 }}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center" }}>
          ...
          <p style={{ fontSize: px(14,compact), color: "rgba(255,255,255,0.55)", fontWeight: 500, lineHeight: 1.5, marginBottom: px(24,compact), maxWidth: "85%" }}>{subtitle}</p>
...
// ══════════════════════════════════════════════════════════
// 2. MAIN SERVICE — identidade / serviço principal
// ══════════════════════════════════════════════════════════
function MainService({ title, subtitle, cta, business_name, color, niche, c: compact }: any) {
  const p = (v: number) => px(v, compact);
  const pad = s(28, compact);
  return (
    <Slide bg="#0D0D0D" compact={compact}>
      {/* Glow diagonal */}
      ...
      {/* Dot grid */}
      <svg style={{ position: "absolute", inset: 0, width: "100%", height: "100%", opacity: 0.06, pointerEvents: "none" }} viewBox="0 0 400 400">
        {Array.from({ length: 8 }, (_, row) => Array.from({ length: 8 }, (_, col) => (
          <circle key={`${row}-${col}`} cx={col * 57 + 28} cy={row * 57 + 28} r="1.5" fill="white"/>
        )))}
      </svg>
      ...
      <div style={{ padding: `${pad}px`, display: "flex", flexDirection: "column", height: "100%", position: "relative", zIndex: 1 }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: px(20, compact) }}>
          {niche && (
            <div style={{ background: `${color}22`, border: `1px solid ${color}40`, borderRadius: px(100,compact), padding: `${s(5,compact)}px ${s(12,compact)}px` }}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center" }}>
          <p style={{ fontSize: px(11,compact), fontWeight: 700, color: `${color}99`, letterSpacing: "0.22em", textTransform: "uppercase", marginBottom: px(10,compact) }}>Destaque</p>
          ...
          <p style={{ fontSize: px(13,compact), color: "rgba(255,255,255,0.50)", fontWeight: 500, lineHeight: 1.55, maxWidth: "88%" }}>{subtitle}</p>
        </div>
...
// 3. PROMOTION
// ══════════════════════════════════════════════════════════
function Promotion({ title, subtitle, cta, business_name, color, c: compact }: any) {
  const p = (v: number) => px(v, compact);
  const pad = s(32, compact);
  return (
    <Slide bg={`linear-gradient(145deg, #0A0D14 0%, ${color}cc 100%)`} compact={compact}>
      {/* Stripes diagonais */}
      <svg style={{ position: "absolute", inset: 0, width: "100%", height: "100%", opacity: 0.06 }} viewBox="0 0 400 400">
        {Array.from({ length: 12 }, (_, i) => <line key={i} x1={i * 50 - 100} y1="0" x2={i * 50 + 300} y2="400" stroke="white" strokeWidth="1.5"/>)}
      </svg>
      {/* Círculo decorativo */}
      <div style={{ position: "absolute", right: px(-60, compact), top: px(-60, compact), width: px(280, compact), height: px(280, compact), borderRadius: "50%", border: "1px solid rgba(255,255,255,0.08)", pointerEvents: "none" }} />
...
// ══════════════════════════════════════════════════════════
// 4. AUTHORITY — autoridade / educativo
// ══════════════════════════════════════════════════════════
function Authority({ title, subtitle, cta, business_name, color, niche, c: compact }: any) {
  const p = (v: number) => px(v, compact);
  const pad = s(28, compact);
  return (
    <Slide bg="#fff" compact={compact}>
      {/* Bloco de cor lateral esquerdo */}
      <div style={{ position: "absolute", left: 0, top: 0, bottom: 0, width: px(80, compact), background: `linear-gradient(180deg, ${color}f0 0%, ${color}c0 100%)` }} />
      {/* Dots no bloco colorido */}
      <svg style={{ position: "absolute", left: 0, top: 0, width: px(80,compact), height: "100%", opacity: 0.12, pointerEvents: "none" }} viewBox="0 0 80 400">
        {Array.from({ length: 6 }, (_, row) => Array.from({ length: 2 }, (_, col) => (
          <circle key={`${row}-${col}`} cx={col * 32 + 16} cy={row * 70 + 35} r="3" fill="white"/>
        )))}
      </svg>

      <div style={{ padding: `${pad}px`, paddingLeft: `${s(80, compact) + s(20, compact)}px`, display: "flex", flexDirection: "column", height: "100%", position: "relative", zIndex: 1 }}>
        {/* Tag */}
        <div style={{ marginBottom: px(16, compact) }}>
          <div style={{ display: "inline-flex", alignItems: "center", gap: px(5,compact), background: `${color}14`, border: `1px solid ${color}30`, borderRadius: px(8,compact), padding: `${s(5,compact)}px ${s(10,compact)}px` }}>
            <span style={{ fontSize: px(9,compact), fontWeight: 900, color, letterSpacing: "0.24em", textTransform: "uppercase" }}>Dica</span>
          </div>
        </div>

        <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center" }}>
          <h2 style={{ fontSize: px(30,compact), fontWeight: 900, color: "#111", letterSpacing: "-0.04em", lineHeight: 1.0, marginBottom: px(12,compact) }}>{title}</h2>
          <div style={{ width: px(32, compact), height: px(3, compact), background: color, borderRadius: px(2,compact), marginBottom: px(12,compact) }} />
          <p style={{ fontSize: px(13,compact), color: "#555", fontWeight: 500, lineHeight: 1.6 }}>{subtitle}</p>
        </div>

        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", paddingTop: px(14,compact), borderTop: "1px solid #f0f0f0" }}>
          <span style={{ fontSize: px(10,compact), color: "#bbb", fontWeight: 600 }}>{business_name}</span>
          <div style={{ display: "inline-flex", alignItems: "center", gap: px(5,compact), background: `${color}12`, borderRadius: px(100,compact), padding: `${s(6,compact)}px ${s(12,compact)}px` }}>
            <span style={{ fontSize: px(10,compact), fontWeight: 800, color, letterSpacing: "0.12em", textTransform: "uppercase" }}>{cta}</span>
          </div>
        </div>
      </div>
    </Slide>
...
// 5. LOCATION
// ══════════════════════════════════════════════════════════
function Location({ title, subtitle, cta, business_name, color, city, c: compact }: any) {
  const p = (v: number) => px(v, compact);
  const pad = s(32, compact);
  return (
    <Slide bg="#181C24" compact={compact}>
      {/* City watermark */}
      {city && <div style={{ position: "absolute", bottom: px(40, compact), left: 0, right: 0, textAlign: "center", fontSize: px(90, compact), fontWeight: 900, color: "rgba(255,255,255,0.025)", letterSpacing: "-0.04em", lineHeight: 1, pointerEvents: "none", userSelect: "none", fontFamily: FONT }}>{city.toUpperCase()}</div>}
      {/* Radial glow */}
      <div style={{ position: "absolute", bottom: 0, left: "50%", transform: "translateX(-50%)", width: px(300, compact), height: px(200, compact), background: `radial-gradient(ellipse, ${color}15 0%, transparent 70%)`, pointerEvents: "none" }} />
      ...
      <div style={{ padding: `${pad}px`, display: "flex", flexDirection: "column", height: "100%", position: "relative", zIndex: 1 }}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center" }}>
          ...
          <h2 style={{ fontSize: px(32, compact), fontWeight: 900, color: "#fff", letterSpacing: "-0.04em", lineHeight: 1.0, marginBottom: px(10, compact) }}>{title}</h2>
          <p style={{ fontSize: px(14, compact), color: "rgba(255,255,255,0.55)", fontWeight: 500, lineHeight: 1.5, marginBottom: px(24, compact) }}>{subtitle}</p>
...
// ══════════════════════════════════════════════════════════
// 6. OFERTA — oferta com urgência
// ══════════════════════════════════════════════════════════
function Oferta({ title, subtitle, cta, business_name, color, c: compact }: any) {
  const p = (v: number) => px(v, compact);
  const pad = s(32, compact);
  return (
    <Slide bg="#0A0D14" compact={compact}>
      {/* Borda esquerda grossa */}
      <div style={{ position: "absolute", left: 0, top: 0, bottom: 0, width: px(6, compact), background: `linear-gradient(180deg, ${color} 0%, ${color}55 100%)` }} />
      {/* Glow top-right */}
      <div style={{ position: "absolute", right: px(-40,compact), top: px(-40, compact), width: px(200, compact), height: px(200, compact), borderRadius: "50%", background: `radial-gradient(circle, ${color}18 0%, transparent 70%)`, pointerEvents: "none" }} />

      <div style={{ padding: `${pad}px ${pad}px ${pad}px ${pad + s(10,compact)}px`, display: "flex", flexDirection: "column", height: "100%", position: "relative", zIndex: 1 }}>
        {/* Badge OFERTA */}
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center" }}>
          ...
          {/* Checklist */}
          {["Qualidade garantida", "Atendimento personalizado"].map((item) => (
            <div key={item} style={{ display: "flex", alignItems: "center", gap: px(8,compact), marginBottom: px(8,compact) }}>
              ...
              <span style={{ fontSize: px(12,compact), color: "rgba(255,255,255,0.60)", fontWeight: 500 }}>{item}</span>
            </div>
          ))}
        </div>
...
// ══════════════════════════════════════════════════════════
// 7. AGENDA — horários disponíveis
// ══════════════════════════════════════════════════════════
function Agenda({ title, subtitle, cta, business_name, color, c: compact }: any) {
  const p = (v: number) => px(v, compact);
  const pad = s(28, compact);
  const days = [
  ...
  return (
    <Slide bg="#0D1117" compact={compact}>
      {/* Top gradient bar */}
      <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: px(3, compact), background: `linear-gradient(90deg, ${color} 0%, ${color}55 100%)` }} />
      {/* Subtle glow */}
      <div style={{ position: "absolute", top: "30%", left: "50%", transform: "translateX(-50%)", width: px(300, compact), height: px(200, compact), background: `radial-gradient(ellipse, ${color}14 0%, transparent 70%)`, pointerEvents: "none" }} />
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: px(6, compact), marginBottom: px(16, compact) }}>
            {days.map((item) => (
              <div key={item.d} style={{
                ...
                background: item.active ? `${color}14` : "rgba(255,255,255,0.04)",
                border: `1px solid ${item.active ? color + "30" : "rgba(255,255,255,0.06)"}`,
                borderRadius: px(8, compact), padding: `${s(7,compact)}px ${s(10,compact)}px`,
              }}>
                ...
                <span style={{ fontSize: px(11, compact), fontWeight: 500, color: item.active ? "rgba(255,255,255,0.60)" : "rgba(255,255,255,0.20)", flex: 1, textAlign: "center" }}>{item.slots}</span>
...
// ══════════════════════════════════════════════════════════
function Depoimento({ title, subtitle, cta, business_name, color, city, c: compact }: any) {
  const p = (v: number) => px(v, compact);
  const pad = s(32, compact);
  return (
    <Slide bg={`linear-gradient(160deg, ${color}f5 0%, ${color}cc 100%)`} compact={compact}>
      {/* Círculos decorativos */}
      <div style={{ position: "absolute", bottom: px(-50, compact), right: px(-50, compact), width: px(220, compact), height: px(220, compact), borderRadius: "50%", border: "1px solid rgba(255,255,255,0.15)", pointerEvents: "none" }} />
      ...
      <div style={{ padding: `${pad}px`, display: "flex", flexDirection: "column", height: "100%", position: "relative", zIndex: 1 }}>
        ...
        <div style={{ position: "relative" }}>
          <div style={{ position: "absolute", top: px(-8, compact), left: px(-4, compact), opacity: 0.20 }}>
            {Icons.quote(s(32, compact), "#fff")}
          </div>
          <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center", paddingLeft: px(8, compact) }}>
            <p style={{ fontSize: px(20, compact), fontWeight: 700, color: "#fff", lineHeight: 1.4, letterSpacing: "-0.02em", marginBottom: px(20, compact) }}>
              "{title}"
            </p>
            <p style={{ fontSize: px(13, compact), color: "rgba(255,255,255,0.70)", fontWeight: 500, lineHeight: 1.5, marginBottom: px(20, compact) }}>{subtitle}</p>
          </div>
        </div>
        ...
        <div style={{ background: "rgba(255,255,255,0.15)", backdropFilter: "blur(10px)", border: "1px solid rgba(255,255,255,0.20)", borderRadius: px(12, compact), padding: `${s(12,compact)}px ${s(16,compact)}px`, display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: px(16, compact) }}>
          ...
          <div style={{ display: "flex", gap: px(2,compact) }}>
            {Array.from({ length: 5 }, (_, i) => <span key={i}>{Icons.star(s(10, compact))}</span>)}
          </div>
        </div>
...
// ══════════════════════════════════════════════════════════
// 9. COMPARAÇÃO — antes vs depois / benefícios
// ══════════════════════════════════════════════════════════
function Comparacao({ title, subtitle, cta, business_name, color, c: compact }: any) {
  const p = (v: number) => px(v, compact);
  const pad = s(28, compact);
  return (
    <Slide bg="#fff" compact={compact}>
      {/* Divisor vertical central */}
      <div style={{ position: "absolute", left: "50%", top: 0, bottom: 0, width: px(1, compact), background: "#f0f0f0", pointerEvents: "none" }} />

      <div style={{ padding: `${pad}px`, display: "flex", flexDirection: "column", height: "100%", position: "relative", zIndex: 1 }}>
        {/* Labels */}
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: px(12, compact), marginBottom: px(16, compact) }}>
          <div style={{ background: "#f5f5f5", borderRadius: px(8, compact), padding: `${s(8,compact)}px`, textAlign: "center" }}>
            <span style={{ fontSize: px(10, compact), fontWeight: 800, color: "#aaa", letterSpacing: "0.18em", textTransform: "uppercase" }}>Sem nós</span>
          </div>
          <div style={{ background: `${color}15`, borderRadius: px(8, compact), padding: `${s(8,compact)}px`, textAlign: "center" }}>
            <span style={{ fontSize: px(10, compact), fontWeight: 800, color, letterSpacing: "0.18em", textTransform: "uppercase" }}>Com a gente</span>
          </div>
        </div>

        {/* Comparison rows */}
        {[
          ["Sem plano", "Com plano mensal"],
          ["Improvisa", "Conteúdo pronto"],
          ["Perde clientes", "Fideliza clientes"],
        ].map(([antes, depois], i) => (
          <div key={i} style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: px(8, compact), marginBottom: px(8, compact) }}>
            <div style={{ background: "#fafafa", borderRadius: px(8, compact), padding: `${s(8,compact)}px ${s(10,compact)}px`, display: "flex", alignItems: "center", gap: px(6,compact) }}>
              <div style={{ width: px(4, compact), height: px(4, compact), borderRadius: "50%", background: "#ddd", flexShrink: 0 }} />
              <span style={{ fontSize: px(11, compact), color: "#aaa", fontWeight: 600 }}>{antes}</span>
            </div>
            <div style={{ background: `${color}10`, border: `1px solid ${color}20`, borderRadius: px(8, compact), padding: `${s(8,compact)}px ${s(10,compact)}px`, display: "flex", alignItems: "center", gap: px(6,compact) }}>
              {Icons.check(s(10,compact), color)}
              <span style={{ fontSize: px(11, compact), color: "#333", fontWeight: 700 }}>{depois}</span>
            </div>
          </div>
        ))}
        ...
        <div style={{ marginTop: px(10, compact), textAlign: "center" }}>
          <span style={{ fontSize: px(10, compact), color: "#ccc", fontWeight: 600 }}>{business_name}</span>
        </div>
      </div>
    </Slide>
  );
...
// ══════════════════════════════════════════════════════════
// 10. STRONG CTA — chamada máxima
// ══════════════════════════════════════════════════════════
function StrongCTA({ title, subtitle, cta, business_name, color, c: compact }: any) {
  const p = (v: number) => px(v, compact);
  const pad = s(32, compact);
  return (
    <Slide bg="#0A0D14" compact={compact}>
      ...
      <div style={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%,-50%)", width: px(200, compact), height: px(200, compact), borderRadius: "50%", border: `1px solid ${color}10`, pointerEvents: "none" }} />
      ...
      <div style={{ padding: `${pad}px`, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: "100%", position: "relative", zIndex: 1, textAlign: "center" }}>
        <p style={{ fontSize: px(10,compact), fontWeight: 800, color: color, letterSpacing: "0.28em", textTransform: "uppercase", marginBottom: px(16,compact) }}>{business_name}</p>
        ...
        <p style={{ fontSize: px(14, compact), color: "rgba(255,255,255,0.50)", fontWeight: 500, lineHeight: 1.5, marginBottom: px(32, compact), maxWidth: "80%" }}>{subtitle}</p>

        {/* CTA button grande */}
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: px(10, compact) }}>
...
// EXPORT DEFAULT
// ══════════════════════════════════════════════════════════
export default function PostCard({ template_type, title, subtitle, cta, business_name, primary_color, niche, city, number, unlocked = true, compact = false }: Props) {
  if (!unlocked) return <Locked number={number} compact={compact} />;

  const props = { title, subtitle, cta, business_name, color: primary_color || "#7c3aed", niche, city, c: compact };
